...
public static partial class SporLocations {
	...
	static readonly List<string> ListFld = ["ID", "CountryCode", "OrgID", "OrgName", "En", "Lt", "Inactive"];
	static readonly List<string> ListSel = ["ID", "CountryCode", "OrgID", "OrgName", "En", "Lt", "Inactive"];
...
public static partial class Spor {
    ...
    public static async Task List(HttpContext ctx, int page = 1, int top = 25, string? parent = null, string? order = null) {
        var m = await new DBPagingRequest<Evrk_Item>("public.v_app_evrk") {
            Limit = top.Limit(1000),
            Page = page,
            Sort = order ?? "ID",
            Desc = ctx.ParamTrue("desc"),
            Where = string.IsNullOrEmpty(parent) ? ctx.ParamTrue("l1") || parent == "" ? new() { Layer = 1 } : null : new() { Parent = parent },
            Fields = ListFld,
            Select = ListSel
        }.Execute();
        await ctx.Response.WriteAsJsonAsync(m);
    ...
    public static async Task Details(HttpContext ctx, string id) {
        ...
        if (await rdr.ReadAsync()) {
            await ctx.Response.WriteAsJsonAsync(new Evrk_Item() {
                ID = rdr.GetString(0),
                Sekcija = rdr.GetString(1),
                Kodas = rdr.GetStringN(2),
                Parent = rdr.GetStringN(3),
                Layer = rdr.GetInt32(4),
                Pavad = rdr.GetString(5),
                Last = rdr.GetBoolean(6)
            });
        }
